...
import json
import pandas as pd
from config import column_config_home
...
# function to fetch all ETFs details from yahoo finance
def fetch_etf_details(ticker):
    stock = yf.Ticker(ticker)
    return stock.info
...
def calculate_total_value(portfolio):
    ...
    for bucket in portfolio["buckets"]:
        for etf in bucket["etfs"]:
            ...
            shares = etf.get(
                "shares", 0
            )  # Use get() to provide a default value of 0 if 'shares' is not present
            total_value += fetch_current_price(ticker) * shares
    ...
    for bucket in portfolio["buckets"]:
        ...
        for etf in bucket["etfs"]:
            ...
            if current_price is not None:
                ...
                etf_data.append(
                    [ticker, etf_name, etf["allocation"], etf["shares"], "N/A", "N/A"]
                )
        ...
        st.dataframe(
            ...
            hide_index=True,
            column_config=column_config_home,
        )
